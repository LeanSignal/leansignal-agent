...
// leansignallogdemandfilter/selector.go
//
// A small parser + evaluator for LogQL stream selectors. lean-api sends
// selectors in a canonical normalized form; LogQL stream selectors are
// syntactically identical to Prometheus vector selectors, and matching
// semantics here follow Prometheus exactly. Hand-rolling this tiny grammar
// avoids a dependency on the prometheus/prometheus module.
...
// matches evaluates the matcher against the given stream labels with
// Prometheus semantics: an absent label is treated as the empty string, so
// `label=""` — and any regex that matches the empty string — matches a stream
// without that label.
...
// selector is one parsed stream selector; ALL of its matchers must match.
...
// parseSelector parses the canonical normalized stream-selector grammar:
//
...
// The empty selector "{}" is rejected: it would match every stream, which
...
// parseLabelName scans a Prometheus label name: [a-zA-Z_][a-zA-Z0-9_]*.
